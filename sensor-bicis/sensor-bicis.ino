...
loop ()
{
  ...
  if (!client.connected ()
      && (millis () - lastConnectionTime > postingInterval) && the_tally > 0)
    {
      client.stop ();
      String reporte =
  ...
  if (diferencia > umb)
      ...
      if (DEBUG)
	{
      ...
      if (strike_number == 0 && is_measuring == 0)
	{			// FIRST HIT
	  Serial.println ("");
      ...
      if (strike_number == 1 && is_measuring == 1)
	{			// SECOND HIT
   ...
   /**/
    ...
    while (analogRead (A0) > the_max && is_measuring == 1)
    {				//is being pressed, in all cases. to measure the max pressure.
      the_max = analogRead (A0);
    }

  //3 - TUBE IS RELEASED
  diferencia = (float) analogRead (A0) / (float) ma;
  if (diferencia < umb && count_this == 0)
    {				//released by either wheel
